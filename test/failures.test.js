...
test('node agent does not crash the demo app', async (t) => {
  ...
  process.env.VULNMAP_HOMEBASE_URL = 'http://localhost:8000/api/v1/beacon';
...
test('node agent does not crash the demo app', async (t) => {
  const BEACON_INTERVAL_MS = 1000;
  process.env.VULNMAP_HOMEBASE_ORIGIN = 'http://localhost:-1';
  process.env.VULNMAP_BEACON_INTERVAL_MS = BEACON_INTERVAL_MS;
  process.env.VULNMAP_SNAPSHOT_INTERVAL_MS = 200;
  // 0: let the OS pick a free port
  process.env.PORT = 0;
